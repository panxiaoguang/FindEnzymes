...
function SearchRebase(dna::String, site_name::String, outF::String)
    ...
    for line in eachline(tmp_file)
        ...
        opposite_right = dna[(pos-120):(pos-50-1)]
        same_right = dna[(pos+15+1):(pos+40)]
        same_left = dna[(pos-40):(pos-15-1)]
        opposite_left = dna[(pos+50+1):(pos+120)]
        if tmpEnzymeDict["forward"] != [(orient=0, start =0, seq=dna"NNN")]
            for element in tmpEnzymeDict["forward"]
                ...
                if element.orient == 1
                    result2 = find_restriction_site(same_right, element.seq)
                    for (name,seq) in oldEnzymes
                        result1 = find_restriction_site(opposite_right, seq)
                    ...
                    result2 = find_restriction_site(same_left, element.seq)
                    for (name,seq) in oldEnzymes
                        result1 = find_restriction_site(opposite_left, seq)
                        if result1 != Dict("forwad"=>[0], "reverse"=>[0]) && result2 != Dict("forwad"=>[0], "reverse"=>[0])
                            extraInfo = extraInfo * ":" * name * ","
                            break
                        end
...
main("REs.tsv","Raw_sequences.fasta","firstStep/cond5")
